...
import { CLOUDINARY_URL, CLOUD_NAME, UPLOAD_PRESET, backend_url } from "../../store/keyStore";
...
const UploadDocument = () => {
    ...
    const uploadToCloudinary = async (file) => {
        ...
        formData.append("cloud_name", CLOUD_NAME);       // Replace with your actual cloud name
        formData.append("resource_type", "auto");               // 👈 Key change: allows any file type
    ...
    const handleUpload = async (e) => {
        ...
        try {
            ...
            const response = await fetch(`${backend_url}/dfs/upload-document`, {
                ...
                credentials: "include", // To send token in cookie
            ...
            setDocuments([result.file, ...documents]); // Append the returned file from backend
            setFile(null);
            setFileTitle("");
                            ...
                            <div className="card-body d-flex flex-column justify-content-between">
                                <div>
                                    ...
                                    <div className="mb-3">
                                        ...
                                        <span className={`badge rounded-pill bg-${getStatusColor(doc.status)} text-uppercase`}>
                                            {doc.status}
                                        </span>
                                    </div>
                                </div>
                                ...
                                {selectedDoc.forwardingTrail.length === 0 ? (
                                    ...
                                    selectedDoc.forwardingTrail.map((entry, idx) => (
                                        <div key={idx} className="p-3 border rounded mb-3 bg-light">
                                            ...
                                            <p className="mb-0 text-muted small"><strong>Time:</strong> {entry.timestamp}</p>
                                        </div>
                                    ))
                                )}
                            </div>
