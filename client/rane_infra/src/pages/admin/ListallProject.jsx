...
import {
  ...
  FaExternalLinkAlt, FaSignature, FaClock, FaInfoCircle,
} from 'react-icons/fa';
...
export default function ListallProject() {
  const [projects, setProjects] = useState([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState('');
  const [openIds, setOpenIds] = useState({});

  // ── Fetch all projects (service logic inlined) ──────────────────────────
  const fetchProjects = async () => {
    setLoading(true);
    setError('');
    try {
      const token = useAuthStore.getState()?.token;
      const res = await fetch(`${backend_url}/project/all`, {
        method: 'GET',
        headers: {
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <div>
            ...
            <div style={{ fontSize: 11, color: 'var(--text-muted)', marginTop: 1 }}>
              {loading ? 'Loading…' : `${projects.length} project${projects.length === 1 ? '' : 's'} · tap a card for full details`}
            </div>
          </div>
        </div>
        <button
