...
import {
  ...
  FiChevronDown, FiChevronUp, FiExternalLink, FiInbox, FiCheckCircle,
  FiUser, FiPaperclip, FiArrowRight, FiTrash2,
} from 'react-icons/fi';
...
const AllDFSRequests = () => {
  ...
  const fetchFiles = async () => {
    try {
      const response = await fetch(`${backend_url}/dfs/files`, { method: 'GET', credentials: 'include' });
  ...
  const handleDeleteClick = (file) => { setSelectedFile(file); setShowDeleteModal(true); };

  const handleDeleteConfirm = async () => {
    if (!selectedFile) return;
    try {
      const response = await fetch(`${backend_url}/dfs/file/${selectedFile._id}`, { method: 'DELETE', credentials: 'include' });
      if (response.ok) {
      ...
      alert('❌ Failed to delete DFS file: ' + err.message);
    } finally {
  ...
  const filteredFiles = files
    .filter(f =>
      (f.fileTitle?.toLowerCase().includes(searchText.toLowerCase()) ||
        f.uploadedBy?.name?.toLowerCase().includes(searchText.toLowerCase()) ||
        f.currentOwner?.name?.toLowerCase().includes(searchText.toLowerCase())) &&
        ...
        <div style={{ background: 'var(--background)', minHeight: '100vh', fontFamily: 'system-ui, -apple-system, sans-serif', color: 'var(--foreground)', padding: '0 2px' }}>
          ...
          <div style={{ background: 'var(--card)', borderRadius: 14, border: '1px solid var(--border)', boxShadow: '0 2px 10px var(--shadow-color)', overflow: 'hidden', marginBottom: 16 }}>
            ...
            <div style={{ background: 'var(--card)', borderBottom: '1px solid var(--border)', padding: '14px 20px', display: 'flex', alignItems: 'center', gap: 10 }}>
              ...
              <div>
                <div style={{ fontSize: 15, fontWeight: 700, color: 'var(--text-strong)', lineHeight: 1.2 }}>All DFS Requests</div>
                <div style={{ fontSize: 11, color: 'var(--text-muted)', marginTop: 1 }}>Every uploaded document and its full communication trail</div>
              </div>
            </div>
            ...
            <div style={{ padding: '16px 20px' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: 12, marginBottom: 16 }}>
                {[
                  { label: 'Total Documents', value: counts.total, sub: 'All uploads', icon: <FiFileText size={15} /> },
                  { label: 'Pending', value: counts.pending, sub: 'Awaiting first review', icon: <FiInbox size={15} /> },
                  { label: 'In Review', value: counts.review, sub: 'Currently processing', icon: <FiClock size={15} /> },
              ...
              <div style={{ background: 'var(--card)', border: '1px solid var(--border)', borderRadius: 10, padding: '14px 16px', marginBottom: 14 }}>
                ...
                <div style={{ fontSize: 12, color: 'var(--text-muted)', marginBottom: 12 }}>Search by title, uploader or owner, then filter and sort.</div>
                ...
                <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', alignItems: 'flex-end' }}>
                  <div style={{ flex: 1, minWidth: 200 }}>
                    ...
                    <div style={{ position: 'relative' }}>
                      ...
                      <input
                        type="text" placeholder="Search title, uploader or owner…"
              ...
              {loading ? (
                ...
                <div style={{ textAlign: 'center', padding: '40px 20px', background: 'var(--card)', border: '1px solid var(--border)', borderRadius: 10 }}>
                  ...
                  <div style={{ fontSize: 13, color: 'var(--text-muted)' }}>{files.length === 0 ? 'No documents have been uploaded yet.' : 'Try adjusting your search or filters.'}</div>
                </div>
              ) : (
                <>
                  {/* ════ DESKTOP TABLE ════ */}
                      ...
                      <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 820 }}>
                        <thead>
                          <tr>
                            ...
                            <th style={thStyle('center')}>Delete</th>
                          </tr>
                        </thead>
                        <tbody>
                              ...
                              <React.Fragment key={file._id}>
                                ...
                                >
                                  ...
                                  <td style={{ ...tdStyle, maxWidth: 260 }}>
                                    ...
                                    {file.docType && <div style={{ fontSize: 12, color: 'var(--text-muted)' }}>{file.docType}</div>}
                                  </td>
                                  <td style={{ ...tdStyle, whiteSpace: 'nowrap' }}>{file.uploadedBy?.name || '—'}</td>
                                  ...
                                  <td style={{ ...tdStyle, textAlign: 'center' }}>
                                    <button
                                      ...
                                      title="Delete file"
                                      style={{ ...btnStyle, padding: '6px', margin: '0 auto', color: 'var(--destructive)', borderColor: 'var(--destructive-border)' }}
                                    >
                                      <FiTrash2 size={14} />
                                    </button>
                                  </td>
                        ...
                        <div key={file._id} style={{ background: 'var(--card)', border: '1px solid var(--border)', borderRadius: 10, overflow: 'hidden' }}>
                          <div style={{ padding: '12px 14px' }}>
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 10, marginBottom: 8 }}>
                              <div style={{ minWidth: 0 }}>
                                ...
                                {file.docType && <div style={{ fontSize: 12, color: 'var(--text-muted)', marginTop: 2 }}>{file.docType}</div>}
                              </div>
                            ...
                            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px 16px', fontSize: 12, color: 'var(--text-muted)', borderTop: '1px solid var(--border)', paddingTop: 10 }}>
                              <span style={{ display: 'inline-flex', alignItems: 'center', gap: 5 }}><FiUser size={12} color="var(--accent)" /> By: {file.uploadedBy?.name || '—'}</span>
                              <span style={{ display: 'inline-flex', alignItems: 'center', gap: 5 }}><FiUser size={12} color="var(--accent)" /> Owner: {file.currentOwner?.name || '—'}</span>
                          ...
                          <div style={{ display: 'flex', borderTop: '1px solid var(--border)' }}>
                            <button
                              ...
                              style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, padding: '9px', border: 'none', borderRight: '1px solid var(--border)', background: open ? 'var(--secondary)' : 'var(--muted)', color: 'var(--primary)', fontSize: 12.5, fontWeight: 600, cursor: 'pointer' }}
                            >
                              {open ? <><FiChevronUp size={13} /> Hide trail</> : <><FiChevronDown size={13} /> View trail</>}
                            </button>
                            <button
                              onClick={() => handleDeleteClick(file)}
                              style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, padding: '9px 16px', border: 'none', background: 'var(--muted)', color: 'var(--destructive)', fontSize: 12.5, fontWeight: 600, cursor: 'pointer' }}
                            >
